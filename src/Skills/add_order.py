...
class add_order:
    ...
    def run(data):
        ...
        cleaned_order = add_order.clean_order(order)

        cleaned_order = add_order.approximate_order(cleaned_order)

        for o in cleaned_order:
            try:
                o['count'] = int(add_order.text2int(o['count']))
            except ValueError:
                o['count'] = 1
        ...
        data['user'].add_order(cleaned_order)
        if reply != "":
